...
from feature_match.common_f import get_rays_from_uv, ray_to_3D, proj_3D_2D, img_pre, proj_3D_2D_cur, render_surface_color, save_img, uv_to_index
...
class CoSLAM():
    ...
    def tracking_render(self, batch, frame_id, gt_color_prev, gt_depth_prev, i_prev, j_prev, i_cur, j_cur, 
                        gt_depth_prev_batch_sift, gt_depth_batch_sift, gt_color_c, gt_depth, gt_color, W0, H0, W1, H1, uv_cur, uv_prev, gt_color_clone, m_i, m_j):
        '''
        ...
        Params:
            ...
            i_prev, j_prev, i_cur, j_cur, gt_depth_prev_batch_sift, gt_depth_batch_sift for 3d 2d


        - for loop with optimization - loss output 
        - can do calculation before or after without repeating it (cv)
        '''
        device = self.device
        ...
        if self.config['tracking']['iter_point'] > 0:
            ...
            prev_c2w = self.est_c2w_data[frame_id]
        ...
        for i in range(self.config['tracking']['iter']):
            ...
            # print("rgb RENDEREDDDDDDDD ", ret["rgb"])
            # print("rgb RENDEREDDDDDDDD SHAPE ", ret["rgb"].shape)
            ...
            # print("########################### uv_prev_in_cur ", uv_prev_in_cur[:10])
            # print("########################### uv_cur ", uv_cur[:10])

            # print("uv_inPREVtest ", uv_prev_in_cur[:10])
            # print("uv cur ", uv_cur[:10])
            ...
            # change get batch sift for new
            gt_depth_clone_in = gt_depth.reshape(-1)

            index_new = uv_to_index(uv_prev_in_cur, gt_color)
            index_new = index_new.long()  # Convert to long if needed
            depth_new = gt_depth[H0:H1, W0:W1]
            depth_new = gt_depth_batch_sift.reshape(-1)
            depth_new = gt_depth_batch_sift[index_new]  # (n)
            ...
            # print("gt depth bacth sifetsdsfsdfsdf ", gt_depth_batch_sift_changed.shape)
            # index_prev, index_cur,
            # print("target d ", target_d.shape)

            # current 2D color
            ret_own = self.model.forward_other(rays_o_cur, rays_d_cur, gt_depth_batch_sift_changed)
            rgb_rendered = ret_own["rgb"]

            # previous 2D color
            # ret_own_prev = self.model.forward_other(rays_o_prev, rays_d_prev, depth_new)
            # rgb_rendered_prev = ret_own_prev["rgb"].detach()


            # loss_2d_color = huber_loss(rgb_rendered, rgb_rendered_prev, delta=1)
            # print("2D COLOR LOSS ", loss_2d_color)



            loss_2d =  huber_loss(uv_cur, uv_prev_in_cur, delta=1) * 0.0001
    ...
    def run(self):
        ...
        # init the previous color depth and c2w 
        ...
        for i, batch in tqdm(enumerate(data_loader)):
            ...
            # Tracking + Mapping
                ...
                m_i, m_j = torch.meshgrid(torch.linspace(
                ...
                self.tracking_render(batch, i, gt_color_prev, gt_depth_prev, i_prev, j_prev, i_cur, j_cur, 
                                     gt_depth_prev_batch_sift, gt_depth_batch_sift, colors_cur, gt_depth, gt_color, W0, H0, W1, H1, uv_cur, uv_prev, gt_color_clone, m_i, m_j)

                # print("INSIDE RUN INDEX: ", i)
            ...
            if rgb is not None:
                ...
                prev_c2w = self.est_c2w_data[i].clone().detach() 
            key = cv2.waitKey(1)
