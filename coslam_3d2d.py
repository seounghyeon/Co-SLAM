...
from feature_match.common_f import get_rays_from_uv, ray_to_3D, proj_3D_2D
...
class CoSLAM():
    ...
    # TRACKER
    def tracking_render(self, batch, frame_id, gt_color_prev, gt_depth_prev, prev_c2w, i_prev, j_prev, i_cur, j_cur, 
                        gt_depth_prev_batch_sift, gt_depth_batch_sift, gt_color_c, gt_depth, gt_color, W1, H0, uv_cur):
        '''
        ...
        for i in range(self.config['tracking']['iter']):
            ...
            uv_prev_in_cur = proj_3D_2D(point_3D_prev, W1, H0, fx, fy, cx, cy, c2w_est, self.device)  # is float
            ...
            loss_2d =  huber_loss_norm(uv_cur, uv_prev_in_cur, delta=1)
            # loss_2d_mean =  huber_loss(uv_cur, uv_prev_in_cur, delta=1)

            # print("loss 2d huber: ", loss_2d)
            # print("loss is ", loss)
            loss = loss + loss_2d
    ...
    def run(self):
        ...
        for i, batch in tqdm(enumerate(data_loader)):
            ...
            # Tracking + Mapping
                ...
                uv_prev, uv_cur, index_prev, index_cur, colors_cur = sift_matcher.match(gt_color_prev_clone, gt_color_clone)
                # print("uv_cur outside: ", uv_cur[:10])
                # print("index ", index_prev)
                ...
                self.tracking_render(batch, i, gt_color_prev, gt_depth_prev, prev_c2w, i_prev, j_prev, i_cur, j_cur, 
                                     gt_depth_prev_batch_sift, gt_depth_batch_sift, colors_cur, gt_depth, gt_color, W1, H0, uv_cur)
